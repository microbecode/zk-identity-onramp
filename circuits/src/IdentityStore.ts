...
export class IdentityStore extends SmartContract {
  ...
  @method initState(initialRoot: Field) {
    this.userPublicKeyRoot.set(initialRoot);
  }

  // TODO: combine this with verification
  @method
  store(leafWitness: MerkleWitnessSet, valueAfter: Field) {
    const rootNow = this.userPublicKeyRoot.get();
    this.userPublicKeyRoot.requireEquals(rootNow);

    // compute the root after incrementing
    const rootAfter = leafWitness.calculateRoot(valueAfter);

    // set the new root
    this.userPublicKeyRoot.set(rootAfter);
  }
